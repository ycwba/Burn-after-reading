...
async function handleRequest(request, env) {
    ...
    if (request.method === 'POST' && pathParts[0] === 'create') {
        try {
            ...
            await env.BURN_MESSAGES_KV.put(id, JSON.stringify(dataToStore), { expirationTtl: 86400 });
    ...
    if (request.method === 'GET' && pathParts[0] === 'read' && pathParts[1]) {
        ...
        try {
            const storedDataString = await env.BURN_MESSAGES_KV.get(id);
            ...
            // 成功读取，从 KV 中删除消息
            await env.BURN_MESSAGES_KV.delete(id);
